...
def parse_ofx(file):
    try:
        ...
        # Vacina para erro de Ledger Balance vazio
        if "<LEDGERBAL>" not in content_text:
            dummy = "<LEDGERBAL><BALAMT>0</BALAMT><DTASOF>20240101000000</DTASOF></LEDGERBAL>"
            if "</STMTRS>" in content_text:
                content_text = content_text.replace("</STMTRS>", f"{dummy}</STMTRS>")
            elif "</BANKTRANLIST>" in content_text:
                content_text = content_text.replace("</BANKTRANLIST>", f"</BANKTRANLIST>{dummy}")
                    ...
                    for line in text.split('\n'):
                        ...
                        if match:
                            try:
                                ...
                                transactions.append({
                                    "Data": match.group(1) + f"/{date.today().year}",
...
# --- LANÇAMENTOS ---
    ...
    with st.form("form_lanc"):
        ...
        c4, c5 = st.columns(2)
        if tipo == "Entrada":
            cats = st.session_state['config_categorias_receita']['Nome'].tolist()
        else:
            cats = st.session_state['config_categorias_despesa']['Nome'].tolist()
        ...
        if st.form_submit_button("Salvar Movimentação"):
            novo = {
                ...
                "Data": dt, "Tipo": tipo, "Conta": conta_sel,
...
# --- SÓCIOS ---
    ...
    tab1, tab2 = st.tabs(["Cadastro", "Histórico"])
    with tab1:
        st.data_editor(st.session_state['socios'], key="edit_socios", num_rows="dynamic", use_container_width=True)
    with tab2:
        ...
        if not df_imp.empty:
            with st.form("conciliacao"):
                ...
                for i, row in df_imp.head(10).iterrows():
                    val = row.get('Valor', 0)
                    tipo_sugerido = "Entrada" if val > 0 else "Saída"
                    
                    st.markdown(f"**{row.get('Data')}** | R$ {val} | {row.get('Descrição')}")
                    col_a, col_b = st.columns(2)
                    
                    cats = st.session_state['config_categorias_receita']['Nome'].tolist() if tipo_sugerido == "Entrada" else st.session_state['config_categorias_despesa']['Nome'].tolist()
                    ...
                    processar.append({
                        "Data": row.get('Data'), "Valor": abs(val), "Tipo": tipo_sugerido,
                        "Conta": conta_destino, "Categoria": cat_sel, "Socio": soc_sel,
                        "Descrição": row.get('Descrição'), "CC": "Geral"
                    })
                ...
                if st.form_submit_button("Confirmar Importação"):
                    for p in processar:
                        if p['Categoria'] != "Ignorar":
                            novo = {
                                "ID": len(st.session_state['financeiro']) + 1,
                                "Data": p['Data'], "Tipo": p['Tipo'], "Conta": p['Conta'],
                                "Categoria": p['Categoria'], "Centro_Custo": p['CC'],
                                "Descrição": p['Descrição'], "Valor": p['Valor'],
                                "Socio": p['Socio'], "Conciliado": "Auto"
                            }
                            st.session_state['financeiro'] = pd.concat([st.session_state['financeiro'], pd.DataFrame([novo])], ignore_index=True)
                    st.success("Conciliado!")

# --- RELATÓRIOS E RECIBOS (MELHORADO) ---
    ...
    with tab_recibos:
        ...
        if not df_entradas.empty:
            ...
            # Tabela editável para selecionar
            df_editado = st.data_editor(
                df_entradas,
                column_config={"Selecionar": st.column_config.CheckboxColumn(required=True)},
                disabled=["ID", "Data", "Tipo", "Conta", "Categoria", "Valor", "Socio"],
                hide_index=True,
            ...
            # AÇÃO 1: Recibo Unificado
            with c_act1:
                if not selecionados.empty:
                    st.write(f"**{len(selecionados)} recibos selecionados.**")
                    ...
                    st.warning("Selecione itens na tabela acima para gerar o PDF Unificado.")

            # AÇÃO 2: Links de WhatsApp Individuais
            with c_act2:
                ...
                if not selecionados.empty:
                    for idx, row in selecionados.iterrows():
                        ...
                        col_z1.write(f"Recibo ID {row['ID']} - {row['Socio']}")
                        
                        if len(num_limpo) >= 10:
                            link = f"https://wa.me/55{num_limpo}?text=Ola, segue seu recibo referente a {row['Categoria']}."
                            col_z2.markdown(f"[📲 Zap]({link})", unsafe_allow_html=True)
                        else:
                            col_z2.caption("Sem Tel")
                            
                        # Botão de download individual também
                        pdf_single = gerar_recibo_unico_pdf(row)
                        col_z2.download_button("⬇️", data=pdf_single, file_name=f"rec_{row['ID']}.pdf", mime="application/pdf", key=f"btn_{row['ID']}")
        else:
    ...
    with tab_relatorios:
        ...
        tipo_view = st.radio("Visualizar por:", ["Detalhado (Extrato)", "Resumo por Categoria", "Resumo por Centro de Custo"], horizontal=True)
        
        if not df_filt.empty:
            if tipo_view == "Detalhado (Extrato)":
    ...
    with tab_balancete:
        ...
        # Simples Balanço de Entradas vs Saídas agrupado
        ...
        if not df_bal.empty:
            ...
            st.divider()
            st.write("**Detalhamento por Conta (Onde está o dinheiro?)**")
...
# --- CONFIGURAÇÕES ---
    ...
    with t1: st.data_editor(st.session_state['config_contas'], num_rows="dynamic", key="cfg_conta")
    with t2: st.data_editor(st.session_state['config_categorias_receita'], num_rows="dynamic", key="cfg_rec")
    with t3: st.data_editor(st.session_state['config_categorias_despesa'], num_rows="dynamic", key="cfg_desp")
    with t4: st.data_editor(st.session_state['config_centros_custo'], num_rows="dynamic", key="cfg_cc")
